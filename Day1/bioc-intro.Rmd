...
The *boxplot* is a common plotting tool in R to compare distributions

- one possible use is to compare the values in a vector with respect to a given factor
- as we don't know the clinical variables in our dataset yet (to come later), we have to make something up

```{r}
fac <- rep(c("A","B"),each=172)
boxplot(eValues[1,] ~ fac)

```


A boxplot can also accept a matrix of data frame as an argument
...
- visualise the distribution of the patients ages using a histogram
...
So far we have been able to print out a subset of our data by specifying a set of numeric indices. Lets say we're interested in high-grade tumours, in which case we might not know in advance which rows / columns these correspond to
...
- for now, we will skip over testing assumptions on the data such as requiring a *normal* (Gaussian) distribtion
...
- firstly, we load the `genefilter` package which has a very convenient function
...
However, this isn't immediately useful unless we can recognise the manufacturer probe IDs

- to provide extra annotation to the table, we can *bind* the columns of test statistic with those from the feature matrix
- however, we can only do this in cases where the rows are in direct correspondence


```{r}
all(rownames(tstats) == rownames(features))
rownames(tstats) <- features$Gene.symbol
